...
class ScenariosApi {
    ...
    async create(params, options) {
        const _a = params || {}, { workspaceId } = _a, scenario = __rest(_a, ["workspaceId"]);
        ...
        let localVarRequestOptions = {
            ...
            json: true,
            body: models_1.ObjectSerializer.serialize(scenario, "Scenario")
        };
    ...
    async delete(params, options) {
        const { workspaceId, scenarioId, } = params || {};
            ...
            return new Promise((resolve, reject) => {
                (0, request_1.default)(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        ...
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = models_1.ObjectSerializer.deserialize(body, "ScenarioCollection");
                            resolve(Object.assign({ lastResponse: response.toJSON() }, body));
                        }
                        else {
                            reject(new apis_1.HttpError(body));
            ...
            return new Promise((resolve, reject) => {
                (0, request_1.default)(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        ...
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = models_1.ObjectSerializer.deserialize(body, "Scenario");
                            resolve(Object.assign({ lastResponse: response.toJSON() }, body));
                        }
                        else {
                            reject(new apis_1.HttpError(body));
    ...
    async update(params, options) {
        const _a = params || {}, { workspaceId, scenarioId } = _a, scenario = __rest(_a, ["workspaceId", "scenarioId"]);
        const localVarPath = this.host + '/workspaces/{workspaceId}/scenarios/{scenarioId}'
            .replace('{' + 'workspaceId' + '}', encodeURIComponent(String(workspaceId)))
            .replace('{' + 'scenarioId' + '}', encodeURIComponent(String(scenarioId)));
        let localVarQueryParameters = {};
